...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues contentValues) {
        ...
        if (StringUtils.isEmpty(name)) {
            Toast.makeText(getContext(), "Pet Name is required", Toast.LENGTH_SHORT).show();
            return null;
            //throw new IllegalArgumentException("Pet requires a name");
        }
        if (gender == null || !PetEntry.isValidGender(gender)) {
            Toast.makeText(getContext(), "Select Pet Gender", Toast.LENGTH_SHORT).show();
            return null;
            //throw new IllegalArgumentException("Pet requires valid gender");
        }
        if (null != weight && weight < 0) {
            Toast.makeText(getContext(), "Enter correct weight", Toast.LENGTH_SHORT).show();
            return null;
        ...
        if(newRowId == -1){
            Log.e(TAG, "insertPet: Failed to insert row for URI "+uri);
            return null;
        }
